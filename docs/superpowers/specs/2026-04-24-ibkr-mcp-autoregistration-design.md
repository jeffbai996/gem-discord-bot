...
`src/tools/ibkr-briefing.ts` is a stub. The real IBKR tool surface already exists as an MCP server at `~/repos/ibkr-terminal/server_http.py` — 32 read-only tools (briefing, positions, margin, what-if, stress test, technicals, etc.) over streamable HTTP on port 8000. Gemma runs on the same box as IBKR MCP (fragserv, per user confirmation).
...
- Registering other MCP servers (jules-review, etc.) — same pattern, future work.
