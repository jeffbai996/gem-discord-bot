...
**Goal:** Replace the broken MCP-based `gemini-discord-mcp` with a standalone Bun daemon ("Gemma") that reads Discord via `discord.js`, calls the Gemini API on allowed messages, and replies with text + optional emoji reactions. Deployed as a systemd user service on fragserv.

**Architecture:** Single long-running Bun process. `discord.js` v14 client pushes `messageCreate` events through an access filter; on pass, attachments are downloaded while history is fetched in parallel; both feed a prompt builder that calls Gemini with `responseSchema` enforcing `{ react, reply }`; reactions and chunked text replies are dispatched in parallel. All state lives in `~/.gemini/channels/discord/` on fragserv. No database, no MCP scaffolding.
...
```bash
cd /Users/jeffbai/repos/gemini-discord-mcp
...
No code changes. This task verifies the bot actually runs and responds in Discord before we touch fragserv.
...
## Task 11: Deploy to fragserv

**Files:**
- Create (on fragserv): `~/.config/systemd/user/gemma.service`
- Create (on fragserv): `~/.gemini/channels/discord/.env`, `access.json`
...
- [ ] **Step 2: Clone/pull on fragserv**

```bash
ssh baila@fragserv 'wsl -u jbai -e bash -lc "cd ~/repos && (test -d gemini-discord-mcp || git clone git@github.com:jeffbai996/gemini-discord-mcp.git) && cd gemini-discord-mcp && git pull && bun install"'
```

Expected: no errors, `bun.lockb` present, `node_modules` populated.

- [ ] **Step 3: Create state dir + secrets on fragserv**

```bash
ssh baila@fragserv 'wsl -u jbai -e bash -lc "mkdir -p ~/.gemini/channels/discord && chmod 700 ~/.gemini/channels/discord"'
```

Then manually (interactively — don't commit tokens to anywhere) populate `.env`:

```bash
ssh baila@fragserv 'wsl -u jbai -e bash -lc "cat > ~/.gemini/channels/discord/.env"'
# paste:
# DISCORD_BOT_TOKEN=...
# GEMINI_API_KEY=...
# (Ctrl+D)
ssh baila@fragserv 'wsl -u jbai -e bash -lc "chmod 600 ~/.gemini/channels/discord/.env"'
```

And `access.json`:

```bash
ssh baila@fragserv 'wsl -u jbai -e bash -lc "cat > ~/.gemini/channels/discord/access.json"'
...
```bash
ssh baila@fragserv 'wsl -u jbai -e bash -lc "mkdir -p ~/.config/systemd/user && cat > ~/.config/systemd/user/gemma.service <<EOF
[Unit]
...
```bash
ssh baila@fragserv 'wsl -u jbai -e bash -lc "sudo loginctl enable-linger jbai"'
ssh baila@fragserv 'wsl -u jbai -e bash -lc "systemctl --user daemon-reload && systemctl --user enable --now gemma"'
```
...
Nothing committed in this task — it's all ops. If you hit any issues that required code changes in gemma.ts, commit those as a follow-up fix and `systemctl --user restart gemma` on fragserv.
