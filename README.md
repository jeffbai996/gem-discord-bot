...
- **Code:** Lives in the `gemini-discord-mcp` directory (`src/server.ts`).
...
```json
{
  "mcpServers": {
    "discord": {
      "command": "npx",
      "args": ["-y", "tsx", "/path/to/gemini-discord-mcp/src/server.ts"]
    }
  }
}
```
*(Make sure to replace `/path/to/gemini-discord-mcp` with the actual path to this repository).*
